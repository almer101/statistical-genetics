...
Number of variants is 16393 and no data is missing (0%). There is 50 males and 59 females in the dataset.
...
The percentage of monomorphic variants in the dataset is 19.5%. After excluding all monomorphic variants there are 13192 variants left.
...
There are 119 alleles of type A and 99 alleles of type B in the variant. MAF of this variant is 0.454.
...
The MAF more or less follows uniform distribution, at least visually. It is not perfect since there is substantial number of vairants with MAF below 0.1. There is 15.9% of variants with MAF below 0.05, and 6% variants with MAF below 0.01. 
What this means is that there are a lot more variants with near-monomorphic constitution.
...
Theoretically the variation of this statistic depends on number of alleles. In this case it can be in interval [0,0.5]. But in case of 4 alleles it can be anywhere between [0, 0.25]. 
The average expected heterozygosity for this data-set is 0.2985.
...
allele.counts = unique(n.alleles.per.STR)
allele.counts
...
h.expected.str(dataset[,1])

h.expected.per.STR = apply(dataset, 2, h.expected.str)
h.expected.per.STR

summary(h.expected.per.STR)
